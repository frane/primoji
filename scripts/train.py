"""Train a GPT model (125M or 1B) on primoji or BPE tokens.
...
Usage:
    # 125M (default):
    python -m scripts.train --tokenizer primoji
    python -m scripts.train --tokenizer mistral

    # 1B with gradient accumulation:
    python -m scripts.train --tokenizer primoji --model-size 1b --micro-batch 4 --grad-accum 8
"""
...
def train(tokenizer_name: str, data_dir: Path, device: str,
          ...
          v2: bool = False, byte_weight: float = 1.0,
          model_size: str = "125m", grad_accum: int = 1) -> None:
    """Train a GPT model (125M or 1B)."""
    ...
    print(f"\n{'='*60}")
    print(f"Training {model_size.upper()}: {tokenizer_name.upper()}")
    print(f"{'='*60}")
    ...
    t0 = time.time()
    running_loss = 0.0
    amp_device = device if device != "mps" else "cpu"
    ...
    for step in range(1, total_steps + 1):
        ...
        if step % eval_interval == 0:
            ...
            # Periodic checkpoint for 1B (expensive to retrain)
            if model_size == "1b" and step % 5000 == 0:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train GPT (125M or 1B)")
    ...
    train(args.tokenizer, Path(args.data_dir), args.device,
          ...
          model_size=args.model_size, grad_accum=args.grad_accum)
